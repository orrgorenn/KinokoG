...
public final class User extends Life implements Lockable<User> {
    ...
    /**
     * This should be used in unfortunate cases where a method cannot accept a {@link mapleglory.util.Locked<User>} to
     * ensure that the method is called after acquiring the lock. It is preferable to submit a runnable to the
     * {@link mapleglory.server.node.ServerExecutor} to acquire the lock in a separate thread, but sometimes we want our
     * code to run first - e.g. before saving to database.
     *
     * @return true if the current thread has acquired the {@link mapleglory.util.Lockable<User>} object.
     */
